...
import is.this_way.better.filesystem.gui.controllers.FileCreationWindowController;
...
public class FileCreationEventHandler implements EventHandler {
    private FolderTreeItem folder;

    public FileCreationEventHandler(FolderTreeItem folder) {
        this.folder = folder;
    }
    ...
    public void handle(Event event) {
        ...
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/FileCreationWindow.fxml"));
            root = loader.load();

            FileCreationWindowController window = loader.getController();
            window.setFolder(folder);
            window.setCreationLabel("File name");
